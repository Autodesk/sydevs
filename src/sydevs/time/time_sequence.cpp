...
time_sequence::const_iterator time_sequence::lower_bound(const time_point& tp) const
{
    ...
    if (!empty()) {
        ...
        if (p_iter != std::end(partitions_) && p_iter->second == tp) {
            qi = p_iter - std::begin(partitions_);
            ti = p_iter->first;
        }
        else {
            if (p_iter != std::begin(partitions_)) {
                qi = (p_iter - 1) - std::begin(partitions_);
    ...
    return const_iterator(*this, qi, ti);
}
...
time_sequence::const_iterator::const_iterator(const time_sequence& ts, int64 qi, int64 ti)
    : ts_ptr_(&ts)
    , qi_(qi)
    , entry_(ti, ti < ts.size() ? ts.partitions_[size_t(qi)].second + ts.offsets_[size_t(ti)] : time_point())
{    
}
...
{
    ++entry_.first;
    if (qi_ + 1 < int64(ts_ptr_->partitions_.size()) && ts_ptr_->partitions_[qi_ + 1].first == entry_.first) {
        ++qi_; 
    }
    if (entry_.first >= 0 && entry_.first < ts_ptr_->size()) {
        entry_.second = ts_ptr_->partitions_[qi_].second + ts_ptr_->offsets_[entry_.first];
    }
    return *this;
}
...
{
    --entry_.first;
    if (qi_ - 1 >= 0 && entry_.first < ts_ptr_->partitions_[qi_].first) {
        --qi_;
    }
    if (entry_.first >= 0 && entry_.first < ts_ptr_->size()) {
        entry_.second = ts_ptr_->partitions_[qi_].second + ts_ptr_->offsets_[entry_.first];
...
time_sequence::const_iterator& time_sequence::const_iterator::operator+=(int64 rhs)
{
    ...
    if (p_iter != std::end(ts_ptr_->partitions_) && p_iter->first == entry_.first) {
        qi_ = p_iter - std::begin(ts_ptr_->partitions_);
    }
    else {
        if (p_iter == std::begin(ts_ptr_->partitions_)) {
            ...
            qi_ = p_iter - std::begin(ts_ptr_->partitions_) - 1;
        }
    }
    if (entry_.first >= 0 && entry_.first < ts_ptr_->size()) {
        entry_.second = ts_ptr_->partitions_[qi_].second + ts_ptr_->offsets_[entry_.first];
